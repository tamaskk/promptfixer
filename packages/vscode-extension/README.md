...
- **Prompt Fixer** — type a quick request like "fix it please"; the extension reads the project's `CLAUDE.md`, `CLAUDE.local.md`, `AGENTS.md`, nested `CLAUDE.md` files, `.claude/` (commands, agents, skills, rules, settings), the `.mcp.json` server names and the active file/selection, then generates a precise prompt for Claude Code
  - One-click engine switch between two local CLIs, both read-only: **Codex CLI** (`codex exec --sandbox read-only`, uses your existing ChatGPT login) and **Claude CLI** (`claude -p` with editing tools disabled)
...
| `promptsChat.fixer.codexPath` | `codex` | Codex CLI path (auto-detected) |
| `promptsChat.fixer.codexModel` | *(empty)* | Optional `--model` for the Codex CLI |
...
```bash
code --install-extension prompts-chat-0.3.0.vsix
```
